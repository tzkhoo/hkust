...
// Mobile carousel component with sliding animation
function MobileServiceCarousel({ services }: { services: typeof healthServices }) {
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      if (!isAnimating) {
        ...
        // After slide animation, update current index
        setTimeout(() => {
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe) {
      goToSlide((currentIndex + 1) % services.length)
    } else if (isRightSwipe) {
      goToSlide(currentIndex === 0 ? services.length - 1 : currentIndex - 1)
    }
  }
  ...
  return (
    <div className="relative max-w-xs mx-auto">
      {/* Service Card with sliding animation */}
      <div 
        className="relative min-h-[120px] flex items-center justify-center overflow-hidden"
      ...
      >
        {/* Current Card */}
        <button
          className={`glass-card rounded-2xl p-6 w-full max-w-[280px] flex flex-col items-center gap-4 transition-all duration-300 hover:scale-102 active:scale-98 focus:outline-none focus:ring-2 focus:ring-primary/50 absolute ${
            isAnimating ? 'animate-slide-out-left' : ''
          }`}
          onClick={() => {}}
          aria-label={`Connect ${currentService.name}`}
        >
          <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${currentService.color} flex items-center justify-center shadow-lg`}>
            <CurrentIcon className="h-6 w-6 text-white" />
          </div>
          <span className="font-semibold text-base text-center">{currentService.name}</span>
        </button>

        {/* Next Card (for animation) */}
        {isAnimating && (
          <button
            className="glass-card rounded-2xl p-6 w-full max-w-[280px] flex flex-col items-center gap-4 transition-all duration-300 hover:scale-102 active:scale-98 focus:outline-none focus:ring-2 focus:ring-primary/50 absolute animate-slide-in-right"
            onClick={() => {}}
            aria-label={`Connect ${nextService.name}`}
          >
            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${nextService.color} flex items-center justify-center shadow-lg`}>
              <NextIcon className="h-6 w-6 text-white" />
            </div>
            <span className="font-semibold text-base text-center">{nextService.name}</span>
          </button>
        )}
      </div>
      ...
      >
        {services.map((_, index) => (
          <button
            key={index}
            className={`w-2 h-2 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/50 ${
              index === currentIndex 
      ...
      <div className="sr-only" aria-live="polite" aria-atomic="true">
        Item {currentIndex + 1} of {services.length}: {currentService.name}
      </div>
    </div>
  )
}
        ...
        {!hasConnectedServices ? (
          <>
            ...
            <div className="mb-8 overflow-hidden">
              {/* Desktop & Tablet: Original sliding conveyor animation */}
              ...
              {/* Mobile: Single card carousel with sliding animation */}

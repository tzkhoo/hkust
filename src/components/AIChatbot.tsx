...
export function AIChatbot({ isOpen, onClose }: AIChatbotProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center px-2 py-4 backdrop-blur-lg bg-black/30 ${!isOpen ? 'hidden' : ''}`}>
      {/* Background Overlay */}
      <div 
      ...
      {/* Nearly Full-Screen Chatbot Container with slide-up animation */}
      <div className={`
        relative w-full max-w-[calc(100vw-32px)] h-[calc(100vh-32px)] rounded-3xl overflow-hidden
        ${isOpen && !isClosing ? 'ai-chatbot-slide-up' : ''}
        ${isClosing ? 'ai-chatbot-slide-down' : ''}
      `}>
